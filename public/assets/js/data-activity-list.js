...
var url = document
  .querySelector('script[data-id="activity-list-script"][data-url]')
...
var KTDatatableJsonRemoteDemo = function () {
		...
		dataTable = $('.kt-datatable').KTDatatable({
			...
			columns: [
				...
				},
				{
					...
					template: function(item) {
						return '\
						<a href="'+url+'/admin/activities/edit/'+item.id+'" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Edit details">\
    ...
    $(document).on('change', '#block1Switch', function (e) {
        var id = $(this).data('id');
        var action_url = url + '/admin/activities/update-block1/' + id;
        $.ajax({
            url: action_url,
            type: "GET",
            dataType: "json",
	...
	$(document).on('click', '.kt_sweetalert_delete_page', function(event) {
		...
		swal.fire({
		    ...
		    if (result.value) {
		    	...
		    	var action_url = url + '/admin/activities/delete/' + id;
		    	$.ajax({
		    		url: action_url,
		    		type: "DELETE",
		    		dataType: "json",
